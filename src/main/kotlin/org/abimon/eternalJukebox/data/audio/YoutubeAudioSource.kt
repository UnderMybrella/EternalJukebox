...
object YoutubeAudioSource: IAudioSource {
    ...
    override fun provide(info: JukeboxInfo, clientInfo: ClientInfo?): DataSource? {
        ...
        val closest = both.firstOrNull() ?: return logNull("[${clientInfo?.userUID}] Searches for both \"${info.artist} - ${info.title}\" and \"${info.artist} - ${info.title} lyrics\" turned up nothing")
